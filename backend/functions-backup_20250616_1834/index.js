...
// ✅ Caricamento dinamico delle rotte con prefisso /api/
const routes = [
...
routes.forEach(([path, file]) => {
  try {
    ...
    console.log(`📦 Carico route /api/${path} da ${file}`);
    app.use(
      `/api/${path}`,
      (req, res, next) => {
        console.log(`➡️  [${req.method}] /api/${path}`);
        next();
      },
      route
    );
    console.log(`✅ Loaded route /api/${path} → ${file}`);
  } catch (error) {
    console.error(`❌ Failed to load route /api/${path} → ${file}`);
    console.error(error);
  }
});
